...
public class ConferenceController extends ExceptionController {
	...
	public void create(
		...
		/*
		 * Double balance = vosClient.getAccountBalance(userName); if (balance
		 * == null) { log.warn("Error balance (" + balance + ") for user <" +
		 * userName + "> to create conference.");
		 * response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
		 * return; }
		 * 
		 * if (balance < 1.0) { log.warn("Not enough money (" + balance +
		 * ") for user <" + userName + "> to create conference.");
		 * response.sendError(HttpServletResponse.SC_PAYMENT_REQUIRED); return;
		 * }
		 */
	...
	public void scheduleNow(
		...
		conference.setAudioConfId(conferenceId);
		Integer vosPhoneNumber = userDao.getVOSPhoneNumberById(userId);
		DonkeyHttpResponse donkeyResp = donkeyClient.createNoControlConference(
				conferenceId, vosPhoneNumber.toString(),
				conference.getAllAttendeeName(), conferenceId);
		if (null == donkeyResp || !donkeyResp.isAccepted()) {
	...
	// public void list(
	// HttpServletResponse response,
	// @RequestParam(value = "offset", required = false, defaultValue = "1") int
	// offset,
	// @RequestParam(value = "username") String username)
	// throws IOException, DataAccessException, JSONException {
	//
...
//			}
	...
	// private void callAttendee(ConferenceModel conference, AttendeeModel
	// attendee) {
	// // transfer attendee status from Initial to CallWait
	// if (!attendee.statusCall()) {
